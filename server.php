...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($typeuser)) { 
  	array_push($errors, "Choose your type of user"); 
  }
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO users (username, email, password, typeuser) 
...
if (isset($_POST['login_user'])) {
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            $_SESSION['typeuser'] = $result_row['typeuser'];
			
            if ($_SESSION['typeuser'] === 'committee') {
